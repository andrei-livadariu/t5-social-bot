...
class SheetUserRepository(UserRepository):
    ...
    def save_all(self, users: list[User]) -> None:
        ...
        with self.lock.gen_wlock():
            ...
            for user in users:
                ...
                diff = self._table.diff(handle.inner, user)
                if not diff:
                    continue

                # Update the repository directly
                handle.inner = user
                # Queue the user for update in the sheets
                diff_data[user.full_name] = diff

            # Save changes to the sheets as well
            self._table.update_partial_all(diff_data)

    def _load(self, raw_users: list[User]) -> None:
        with self.lock.gen_wlock():
            self.users = UserHandle.wrap_list(raw_users)
